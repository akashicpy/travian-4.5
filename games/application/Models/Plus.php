...
class Plus
{
        ...
        if ($rGold <= $session->gold) {
            if ($this->checkupAll()) {
                for ($i = 1; $i <= 18; $i++) {
                    ...
                    // new logic allows level up all village resources to 10;
                    $maxLvl = 10;
                ...
                return $this->errorMsg('Collect level 10 fields with verb');
            }
        }
    ...
    function buyResources($type)
    {
        ...
        switch ($type) {
            case 0:
                $eNum = $config['resources01A'];
                $gCost = $config['resources01'];
                break;
            case 1:
                $eNum = $config['resources02A'];
                $gCost = $config['resources02'];
                break;
            case 2:
                $eNum = $config['resources03A'];
                $gCost = $config['resources03'];
                break;
        }
    ...
    function moreProtection($type)
    {
        ...
        switch ($type) {
            case 0:
                $bonus = 1;
                $gCost = $config['protect01'];
                break;
            case 1:
                $bonus = 3;
                $gCost = $config['protect02'];
                break;
            case 2:
                $bonus = 6;
                $gCost = $config['protect03'];
                break;
        }
        if ($gCost <= $this->session->gold) {
