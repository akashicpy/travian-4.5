...
class MYSQL_DB extends DB
{
	...
	*****************************************/
	...
	function CheckUniqueIP($ip)
	{
		...
		if (!count($this->query("SELECT id FROM `users` WHERE `email`='" . $exist['email'] . "'"))) {
			return false;
		}
	...
	function generateBase($sector)
	{
		...
		switch ($sector) {
			case 1:
				$queryit = " -4 > x and y > " . rand(-1, -20) . " and fieldtype = 3 and `type_of`='' and occupied = 0";
				$xm = -1;
				$ym = 1;
				break;
			case 2:
	...
	function generateBase2($sector)
	{
		...
		switch ($sector) {
			...
			case 1:
				$queryit = " 0 > x and y > 0 and fieldtype = 3 and occupied = 0 and x>-" . (WORLD_MAX / RADIUS) . " and y<" . (WORLD_MAX / RADIUS) . " and type_of='' ORDER BY RAND() LIMIT 1";
	...
	function checkArtefactsEffects($uid, $vref, $type)
	{
		...
		if ($type == 11) {
			...
			$q = "SELECT size from artefacts where owner=:U and type=:T and activated=1";
		} else {
				...
				if (!MOMENT_TRAIN) {
					...
					if ($timepast <= 0 && $train['amt'] > 0 && $timepast2 >= $train['eachtime']) { //услоinия короч
						...
						if ($unit != 99) {
							...
							if ($unit > 20 && $unit <= 30) {
								$unit = $unit - 20;
							}//если это галлы то обрезаем на 20см
							if ($unit > 10 && $unit <= 20) {
								$unit = $unit - 10;
							}//если это галлы то обрезаем на 10см(а рим не согласен на обрезания *печальный еinрей*
					...
					if ($unit != 99) {
						...
						if ($unit > 60) {
							$unit = $unit - 60;
						} //если БК то делаем обрезание ибо у нас 10юнитная система азаза
	...
	function getAllProd($res, $fdataarr, $plus, $ocounter)
	{
		...
		for ($i = 0; $i <= count($holder) - 1; $i++) {
			$base += ${'bid' . $firstbuild}[$fdataarr[$holder[$i]]]['prod'];
		}
		...
		if ($res == 4) {
			...
			if ($zavod >= 1 || $zavod2 >= 1) {
				...
				if ($zavod) {
					$summa += ${'bid' . $secondbuild}[$zavod]['attri'];
				}

				if ($zavod2) {
					$summa += $bid9[$zavod2]['attri'];
				}
	...
	function GetAllyNoticeforAlly($limit, $aid = 0)
	{ //shitty
	...
	public function getUpkeep($arra, $type, $buildarray, $unpack = 0)
	{
		...
		for ($i = $start; $i <= $end; $i++) {
			...
			if (isset($horsedrinking)) {
				...
				if (
					($i == 4 && $buildarray['f' . $horsedrinking] >= 10)
	...
	***************************/
	...
	function getAvailableExpansionTraining($rasa, $vid)
	{
		...
		if ($residence > 0) {
			...
			if ($palace >= 10 and $palace < 15) {
				...
				$maxslot = 1;
			} elseif ($palace >= 15 and $palace < 20) {
	...
	function procDistanceTime($coor, $thiscoor, $ref, $mode, $vrefka = 0, $oas = 0)
	{
		...
		if ($vrefka == 0) {
			$vrefka = $this->getBaseID($coor['x'], $coor['y']);
		}
	...
	function processProduction($bountywid, $fdata, $info, $oasis, $plus, $upkeep, $her = array())
	{
		...
		if (!count($upkeep)) {
			$upkeep = $this->getUpkeep($this->getAllUnits($bountywid, 0, 1), $plus['tribe'], $fdata);
		} else {
		...
		$zapros = $zap = "";
		if ($info['maxstore'] != $ress) {
			$zapros .= "`maxstore` = '" . $ress . "'";
		}
		if ($info['maxcrop'] != $crop) {
			if (!empty($zapros)) {
				$zap = ",";
			}
			$zapros .= $zap;
...
('" . $uid . "', 0, 0, '7', 0, '2', 0, '100', '0', 0, 0, '4', '1', '10', '" . $time . "', '" . $time . "', '')";
	...
	function addAdventure($wref, $uid, $count)
	{
		...
		$x = $xy['x'];
		$y = $xy['y'];
		if ($x < 0 && $x - 30 > -WORLD_MAX) {
			$xarray = range($x - 30, $x + 30);
		} elseif ($x < 0 && $x - 30 < -WORLD_MAX) {
			$xarray = range($x, $x + 60);
		}
		if ($y < 0 && $y - 30 > -WORLD_MAX) {
			$yarray = range($y - 30, $y + 30);
		} elseif ($y < 0 && $y - 30 < -WORLD_MAX) {
			$yarray = range($y, $y + 60);
		}
		if ($x >= 0 && $x + 30 < WORLD_MAX) {
			$xarray = range($x - 30, $x + 30);
	...
	function FPG($data, $set)
	{
		...
		if (isset($data[$set]) && !is_numeric($data[$set])) {
			die("Ты что,ХаЦкер?:О");
		}
	...
	function bodyClass($agent)
	{
		...
		if (strpos($agent, 'Firefox'))
			$body_browser_class = 'gecko';
		elseif (strpos($agent, 'Chrome'))
			$body_browser_class = 'chrome';
		elseif (strpos($agent, 'Safari'))
			$body_browser_class = 'safari';
		elseif (strpos($agent, 'Opera'))
			$body_browser_class = 'opera';
		elseif (strpos($agent, 'MSIE 6.0'))
			$body_browser_class = 'ie6';
		elseif (strpos($agent, 'MSIE 7.0'))
			$body_browser_class = 'ie7';
		elseif (strpos($agent, 'MSIE 8.0'))
			$body_browser_class = 'ie8';
		elseif (strpos($agent, 'MSIE 9.0'))
			$body_browser_class = 'ie9';
		elseif (strpos($agent, 'MSIE 10.0'))
			$body_browser_class = 'ie10';
		elseif (strpos($agent, 'Trident/7'))
			$body_browser_class = 'ie11';
	...
	function generateFarm($kid, $uid, $username)
	{
		...
		switch ($sector) {
			case 1:
				$queryit = " 0 > x and y > 0 and `fieldtype` = " . rand(1, 12) . " and `type_of`='' and occupied = 0";
				$xm = -1;
				$ym = 1;
				break;
			case 2:
